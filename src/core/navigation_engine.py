...
class NavigationEngine:
    ...
    def __init__(self, google_api_key: str, teams_webhook_url: Optional[str] = None):
        """
        Initialize the navigation engine

        Args:
            google_api_key: Google API key for Gemini
            teams_webhook_url: Optional Teams webhook URL for notifications
        """
        ...
        logger.info(
            "NavigationEngine initialized with issue detection and Teams alerting"
        )
    ...
    def _transition_state(self, new_state: NavigationState, reason: str = "") -> None:
        ...
        logger.info(
            f"State: {old_state.value} → {new_state.value} (step {self.current_session.step_count})"
        )
        if reason:
    ...
    def execute_step(self) -> bool:
        ...
        if self.current_session.state not in [
        ...
        try:
            ...
            if not success:
                ...
                logger.warning(
                    f"Action failed (errors: {self.current_session.error_count})"
                )
                ...
                if self.current_session.error_count >= self.current_session.max_errors:
                    ...
                    self.teams_notifier.send_alert(
                        ...
                        f"Objective: {self.current_session.objective}\n"
                        f"Total Steps: {self.current_session.step_count}\n"
                        f"Errors: {self.current_session.error_count}\n"
                        f"Last Issue: {issue.get('title', 'Unknown')}",
                        severity="P0",
                        category="NAVIGATION_ERROR",
                        suggested_fix="Check application health and investigate recent changes.",
                        screenshot_path=screenshot_path,
                        target_url=self.current_session.url,
                        step_number=step,
                    )
                    return False
                else:
    ...
    def run_session(
        ...
        # Check for timeout
        if (
            session.step_count >= max_steps
            and session.state == NavigationState.NAVIGATING
        ):
            self._transition_state(
                NavigationState.TIMEOUT, f"Reached max steps ({max_steps})"
            )
    ...
    def _send_completion_notification(
        ...
        if session.state == NavigationState.COMPLETED:
            ...
            status_emoji = "✅"
            status_text = "SUCCESS"
        elif session.state == NavigationState.ERROR:
            severity = "P1"
            status_emoji = "❌"
            status_text = "FAILED"
        elif session.state == NavigationState.TIMEOUT:
            severity = "P2"
            status_emoji = "⏱️"
            status_text = "TIMEOUT"
        else:
        ...
        # Build description
        description = f"Navigation session {session.session_id} completed with status: {status_text}\n\n"
        description += f"**Objective:** {session.objective}\n"
        description += f"**Duration:** {duration:.1f}s\n"
        description += f"**Steps:** {session.step_count}\n"
        description += f"**Errors:** {session.error_count}\n"

        if session.issues_detected:
            description += f"**Issues Detected:** {len(session.issues_detected)}\n"
            for issue in session.issues_detected[:3]:  # Show first 3 issues
                description += f"- {issue.get('severity', 'P2')}: {issue.get('title', 'Unknown')}\n"
        ...
        self.teams_notifier.send_alert(
            ...
            severity=severity,
            category="NAVIGATION_COMPLETED",
            suggested_fix="Review session details in dashboard for full report.",
            screenshot_path=screenshot_path,
