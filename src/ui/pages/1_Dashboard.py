...
from src.ui.shared_session import (
    ...
    register_session,
    unregister_session,
    clear_session_data as clear_shared_session,
)
...
def run_navigation_session(
    ...
    try:
        ...
        while session.step_count < max_steps and status.get("running", True):
            ...
            if session.actions_taken:
                ...
                if action_type == "click":
                    ...
                    write_log(
                        f"{action_type}: {short_reasoning}", "thinking", full_reasoning
                    )
        ...
        issues = [
            {
                ...
                else str(issue.get("title", "Issue")),
                "description": issue.description
                if hasattr(issue, "description")
                else str(issue.get("description", "")),
                "severity": issue.severity
                if hasattr(issue, "severity")
                else str(issue.get("severity", "P2")),
                "category": issue.category.value
                if hasattr(issue, "category") and hasattr(issue.category, "value")
                else str(issue.get("category", "Unknown")),
                "suggested_fix": issue.root_cause
                if hasattr(issue, "root_cause")
                else str(issue.get("suggested_fix", "")),
            }
        ...
        write_log(
            f"Session completed: {state_value}",
        ...
        if state_value == "completed":
            notifier.send_alert(
                ...
                description=f"Objective: {objective}\nSteps taken: {session.step_count}\nIssues found: {len(issues)}",
                severity="P3",
                category="Session",
                target_url=url,
            )
            write_log("Sent Slack notification for session completion", "info")
        else:
            notifier.send_alert(
                title="Session Failed",
                description=f"Objective: {objective}\nFinal state: {state_value}\nSteps taken: {session.step_count}\nIssues found: {len(issues)}",
                severity="P1",
                category="Session",
                target_url=url,
            )
            write_log("Sent Slack notification for session failure", "info")

        # Send Slack alerts for any P0/P1 issues
        for issue in issues:
            if issue.get("severity") in ["P0", "P1"]:
                notifier.send_issue_alert(issue)
                write_log(f"Sent Slack alert for {issue.get('severity')} issue", "info")
        ...
        notifier.send_alert(
            title="Session Error",
            description=f"An unexpected error occurred during session execution.\n\nError: {str(e)}\n\nObjective: {objective}",
            severity="P0",
            category="Error",
            target_url=url,
        )
        write_log("Sent Slack notification for session error", "info")
...
if len(session_options) > 1:
    selected = st.selectbox(
        ...
        if selected_session_id in session_options
        else 0,
...
with ctrl_col5:
    max_steps = st.number_input(
        "Max Steps", min_value=5, max_value=50, value=20, disabled=is_running
    )
